...
const Movies = () => {
  ...
  const filterMovies = () => {
    if (selectedCategory === 'all') {
      ...
      setDisplayedMovies(movies.filter(m => m.category_id === selectedCategory));
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black">
        <Navbar />
        <div className="pt-24 text-white text-center">Loading movies...</div>
      </div>
    );
  }
              ...
              <div className="aspect-[2/3] rounded-lg overflow-hidden bg-gray-800 transition-transform duration-300 group-hover:scale-105 shadow-lg">
                {movie.stream_icon ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-800 to-gray-900">
                    <p className="text-gray-400 text-sm text-center px-4">{movie.name}</p>
                  </div>
                )}
              </div>
